...
def abrir_clientes():
    ...
    def refrescar():
        ...
        for c in modClientes.cargarClientes():
            tree.insert("", "end", values=(c.idCliente, c.nombre, c.telefono, c.direccion, c.extra, c.servicios))

    def registrar():
        try:
            now = datetime.now()
            idCliente = f"Cli_{now.strftime('%y%m%d_%H%M%S')}"
            cli = modClientes.Cliente(idCliente, entradas[0].get(), int(entradas[1].get()), entradas[2].get(), entradas[3].get())
    ...
    def eliminar():
        ...
        try:
            ...
            if tiene_servicios > 0:
                messagebox.showwarning("No se puede eliminar", f"El cliente '{nombre}' tiene {tiene_servicios} servicio(s) registrado(s).", parent=win)
                return

            if messagebox.askyesno("Confirmar eliminación", f"¿Eliminar al cliente '{nombre}' definitivamente?", parent=win):
    ...
    ttk.Button(win, text="🏠 Volver al menú principal", width=30, style="Volver.TButton", command=lambda: [win.destroy(), ventana.deiconify()]).pack(pady=20)
    refrescar()
...
def abrir_lavadas():
    ...
    # Cambiar estado manualmente con combobox
    def cambiar_estado():
        item = tree.selection()
        if not item:
            messagebox.showwarning("Atención", "Seleccione una lavada.", parent=win)
    ...
    # Cambiar pago manualmente con combobox
    def cambiar_pago():
        item = tree.selection()
        if not item:
            messagebox.showwarning("Atención", "Seleccione una lavada.", parent=win)
    ...
    # Doble clic en fila → alterna pago pendiente/pagado
...
# MENÚ PRINCIPAL
# ------------------------------------------------------------
ttk.Label(ventana, text="🧺 Sistema de Lavandería", font=("Segoe UI", 26, "bold"), foreground=COLOR_PRINCIPAL, background=COLOR_FONDO).pack(pady=40)
...
ttk.Label(ventana, text="© 2025 - Jave’s Laundry\nHecho por: Wicho y Arenz", font=("Segoe UI", 10, "italic"), background=COLOR_FONDO, foreground="#9E9E9E").pack(side=tk.BOTTOM, pady=20)
